...
async def reject_appointment(
    ...
    if not appointment:
        raise HTTPException(status_code=404, detail="Appointment not found")
    print(f"🚨 [REJECT API] Found appointment: status={appointment.get('status')}, patient={appointment.get('patient_id')}, doctor={appointment.get('doctor_id')}")  #arjun
    ...
    if current_status not in ["pending", "approved", "scheduled"]:
        ...
        print(f"🚨 [REJECT API] Invalid status for cancellation: {current_status}") #arjun
    ...
    new_status = "rejected" if is_doctor else "cancelled"    #arjun
    update_data = {
        "status": new_status,
